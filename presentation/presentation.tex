...
\hspace{1cm}uniformly draw sample $D_m$ from dataset $D$ (with repl.)\\
\hspace{1cm}build model $T_m$ on dataset $D_m$ to obtain hypothesis $h_m(x)$\\
combine hypotheses 
...
\rule{10cm}{0.05cm}\\
for base learner $m$ in $\{1,2,...,M\}$\\
\hspace{1cm}uniformly draw sample $k_m$ of size $L$ from features $\{1,2,...,K\}$ \\\hspace{1cm}(with repl.)\\
\hspace{1cm}uniformly draw sample $D_m$ from dataset $D$ (with repl.)\\
\hspace{1cm}build model $T_{m}$ on dataset $D_m$ using feature set $k_m$ \\
...
\frametitle{Boosting}
\begin{itemize}
\item Method proposed by Freund \& Shapire %\cite{freund1996experiments}
...
\rule{10cm}{0.05cm}\\
for base learner $m$ in $\{1,2,...,M\}$\\
\hspace{1cm}train base learner according to weighted data $d^{(m)}$ \\
\hspace{1cm}and obtain hypothesis $h_m:\textbf{x}\mapsto\{-1,+1\}$\\
\hspace{1cm}calculate weighted classifier error\\
\hspace{1cm}$\epsilon_m=\sum_{i=1}^{N}d_i^{m}I(y_i\neq h_t(x_i))$\\
\hspace{1cm}calculate hypothesis weighting $\beta_m=\frac{1}{2}\log(\frac{1-\epsilon_m}{\epsilon_m})$\\
\hspace{1cm}update data weighting, e.g. by \\
\hspace{1cm}$h_m(x_i)=y_i:d_i^{m+1}=d_i^{m}\exp(-\beta_m)$\\
\hspace{1cm}$h_m(x_i)\neq y_i:d_i^{m+1}=d_i^{m}\exp(\beta_m)$ \\
$\hat{y}(x)=H_{final}(x)=\frac{1}{M}\sum_{1}^{M}\beta_mh_m(x)$
\rule{10cm}{0.05cm}\\
}
...
\begin{itemize}
\item Developed by Friedman % \cite{friedman2001greedy}
\item Extended boosting to regression problems
\item Shortcomings of current ensemble is identified by gradients instead of weightings of data
\item In each stage $m$, a new learner improves the current ensemble $H_{m-1}$ and is fitted to $(x_i, y_i - H_{m-1}(x_i)), \forall i \in\{1,2,...,N\}$
\end{itemize}
}
...
\begin{itemize}
\item Advancement of Gradient Boosting, again by Friedman % \cite{friedman2002stochastic}
\item Takes ideas from Bagging: 
\begin{itemize}
...
\item[$+$] ensembling decreases variance and bias
\item[$+$] often scalable
\item[$+$] 
\item[$+$] 
\item[$+$] 
...
\begin{itemize}
\item Scalability
\item Evolving statistical properties %\cite ATHEY
\item ?Out of the black box %\cite???
\end{itemize}
}
